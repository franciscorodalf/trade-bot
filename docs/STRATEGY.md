...
Utilizamos un **Random Forest Classifier** de `scikit-learn`. 
...
- **Entrenamiento Universal**: A diferencia de otros bots que entrenan un modelo por moneda, nosotros usamos un **Modelo Universal**. Entrenamos con datos combinados de BTC, ETH, SOL, etc. Esto permite a la IA aprender patrones generales del mercado cripto ("sentimiento"), haciéndola más adaptable a monedas nuevas.

### Features (Variables de Entrada)
La IA no "ve" el gráfico, ve estos números derivados:
...
No usamos stops fijos (ej. siempre vender a -2%). Usamos el **ATR (Average True Range)**.
